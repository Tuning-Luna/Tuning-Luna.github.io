...
npm run stats:fetch  # regenerate stats.ts + contributions.ts + repoStats.ts (needs gh auth)
...
- CI (`.github/workflows/deploy.yml`) runs lint, check:i18n, and build, then deploys `dist/` to GitHub Pages. **Every run — push to `main`, daily schedule, or manual dispatch — first refreshes the stats snapshots** via `npm run stats:fetch` (using the workflow's `GITHUB_TOKEN`) and commits them back to `main` as `github-actions[bot]` when they changed. `dist/` is gitignored.
...
Two kinds, kept apart on purpose. **Generated** — rewritten in place by `npm run stats:fetch` (scripts/fetch-stats.mts), never edit by hand: `src/data/stats.ts` (account stats), `src/data/contributions.ts` (12-month contribution calendar) and `src/data/repoStats.ts` (per-repository stars/forks/language/archived). **Hand-maintained**: `src/data/profile.ts` URLs/bio, and `src/data/projects.ts`, which only chooses *which* repositories are listed and joins them with `repoStats.ts` — so project-card numbers come from the API instead of a hand-typed copy that drifts. That join is type-checked (`projects.ts` keeps each `name` a literal via `as const satisfies`), so curating a repository the snapshot lacks is a `tsc -b` failure rather than a card without stats. Intentionally not live — no runtime backend. Forks are excluded everywhere; `repoStats.ts` also filters out private repositories, since it is committed to a public repo. The i18n snapshot note interpolates `snapshotDate` from `stats.ts`, so regeneration needs no locale edits.
...
### Layout & sections
`src/App.tsx` composes the page: fixed `.site-bg` → `AppBar` → split layout (above 1200px: two sticky 360px frosted-glass asides — Hero on the left, the PageViews/NowPlaying/MiniPlayer widgets on the right — with the scrolling content column between them; below 1200px: single column, hero → widgets → content). Grids inside `.layout__content` use **container queries**, not viewport media queries. Scroll-reveal is the `useScrollReveal` hook (`src/hooks/`) toggling `.pre-reveal` on `.m3-section`. Sections live in `src/sections/`; section-specific widgets sit beside their consumers (e.g. `TechChip.tsx`, `ProjectCard.tsx` + `ProjectRow.tsx`, `ContributionHeatmap.tsx`), while all static data — including the `techIcons.ts` glyph table the tech chips are typed against — lives in `src/data/`. `src/components/` holds the reusable M3 primitives (Button, Card, Chip, Icon, IconButton, Section, Slider, SmartImage, Stat) plus the app chrome (AppBar, Footer, LanguageToggle, ThemeToggle), all with colocated CSS (`.css` beside each `.tsx`). MiniPlayer is purely presentational — its audio engine (states, fades, volume persistence) is the headless `src/hooks/useAudioPlayer.ts`. `Section.tsx` is the shared wrapper (eyebrow + title + subtitle). Remote or slow-loading images (GitHub avatar, third-party cards/covers) go through `SmartImage` — skeleton while loading, tonal fallback on error — never a bare `<img>`.
