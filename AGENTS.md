...
### Layout & sections
`src/App.tsx` composes the page: fixed `.site-bg` → `AppBar` → split layout (above 1000px: sticky 360px frosted-glass hero `aside` on the right, scrolling content on the left; below 1000px: single column). Grids inside `.layout__content` use **container queries**, not viewport media queries. Scroll-reveal is an IntersectionObserver in `App.tsx` toggling `.pre-reveal` on `.m3-section`. Sections live in `src/sections/`; some (`Activity`, `TechStack`) are commented out in `App.tsx` but still on disk. Reusable M3 components live in `src/components/` with colocated CSS (`.css` beside each `.tsx`). `Section.tsx` is the shared wrapper (eyebrow + title + subtitle).
