...
Two kinds, kept apart on purpose: **generated** (`src/data/stats.ts` account stats and `src/data/contributions.ts` 12-month contribution calendar — both rewritten in place by `npm run stats:fetch`, scripts/fetch-stats.mts; never edit them by hand) and **hand-maintained** (`src/data/profile.ts` for URLs/bio, `src/data/projects.ts` for star/fork counts). Intentionally not live — the site has no runtime backend. Forks are excluded everywhere. The footer's snapshot note interpolates `snapshotDate` from `stats.ts`, so regenerating needs no locale edits.
